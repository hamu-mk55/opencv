...
class ImageResults:
    def __init__(self, ksize, max2min, offset):
        self.ksize = ksize
...
def diff_split(img):
    def _split_imgs_by_freq(img_org, ksize, img_out_offset=None):
        img_base = cv2.blur(img_org, ksize=(ksize, ksize))
    ...
    # cv2.imwrite("img31.jpg", img_out)
    # print(f"{max2min}")

    results_list.append(ImageResults(ksize=31, max2min=max2min, offset=0))

    # 2nd
    for cnt, ksize in enumerate(range(21, 0, -2)):
        img_base, img_diff, img_out, max2min = _split_imgs_by_freq(img_diff, ksize=ksize, img_out_offset=128)

        # cv2.imwrite(f"img{ksize:02d}.jpg", img_out)
        # print(f"{max2min}")

        results_list.append(ImageResults(ksize=ksize, max2min=max2min, offset=128))
...
def diff_merge(img, res_list):
    def _merge_imgs_by_freq(img_org, img_out, res: ImageResults):
        ...
        img_base = cv2.blur(img_org, ksize=(res.ksize, res.ksize))
        img_diff = img_org - img_base
...
if __name__ == '__main__':
    ...
    # 学習
    img11 = cv2.GaussianBlur(img, ksize=(11, 11), sigmaX=0)
    ...
    res_list = diff_split(img11)

    # 復元処理
    img22 = cv2.GaussianBlur(img, ksize=(31, 31), sigmaX=0)
